...
$q = mysqli_query($konek, "SELECT * FROM kategori WHERE kategori_id = $id LIMIT 1");
if (!$q || mysqli_num_rows($q) === 0) {
    $_SESSION['flash'] = ['type' => 'warning', 'msg' => 'Data kategori tidak ditemukan.'];
...
<div class="container-fluid page-bg">
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-light">Ubah Kategori</h1>
    <a class="btn btn-primary" href="kategori_data.php">Kembali</a>
  </div>
        ...
        <div class="mb-3 row">
          ...
          <div class="col-sm-10">
            <input type="text" name="nama_kategori" class="form-control form-control-dark" required
                   value="<?= isset($_POST['nama_kategori']) ? htmlspecialchars($_POST['nama_kategori']) : htmlspecialchars($data['nama_kategori']) ?>">
          </div>
        </div>
        <div class="mb-3 row">
          <label class="col-sm-2 col-form-label label-light">Keterangan</label>
          <div class="col-sm-10">
            <textarea name="keterangan" class="form-control form-control-dark" rows="4"><?= isset($_POST['keterangan']) ? htmlspecialchars($_POST['keterangan']) : htmlspecialchars($data['keterangan']) ?></textarea>
          </div>
        </div>
